...
function prepareInterviewData_asq() {
  ...
  var gate_info  = JSON.parse(Gate_Info);
  var selected_flights  = JSON.parse(Selected_Flights);
  ...
  for (i = 0; i < today_flight_list_asq.length; i++) {
    ...
    //get gate info
    flight.display_time_at_gate = "Not available";
    for (j = 0; j < gate_info.length; j++) {
      ...
      if ((gate.Flight == flight.Flight) && (gate_date == flight.Date))
      {
        ...
        flight.display_terminal = gate.display_terminal;
        
        flight.display_date = flight.Date;

        flight.display_time = gate.display_time;
    ...
    for (j = 0; j < selected_flights.length; j++) {
      ...
      {
        flight.selected = "Planned";
        console.log("Planned: ", selected_flight.Flight);
       }
    }
